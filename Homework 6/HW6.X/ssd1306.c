...
void ssd1306_drawChar(int pos_x, int pos_y, unsigned char letter){ // use drawPixel to display an ASCII character
    unsigned char pix;
    int i=0,j=0;
    while (i<8){
        while (j<5){
           pix = ((ASCII[letter-0x20][j])>>i)&1;
           if (pix==1){
               ssd1306_drawPixel(j+pos_x,i+pos_y,1);
           }
           if (pix==0){
               ssd1306_drawPixel(j+pos_x,i+pos_y,0);
           }
           j++;
        }
        j=0;
...
void ssd1306_drawMessage(int init_x, int init_y, unsigned char *message) {
    int hori_max=128, verti_max=32; // maximum horizontal and vertical positions
    int i = init_x, j = init_y; // initial positions passed into function
    if (i>=hori_max && j>=verti_max){ // check initial positions
        return;
    }
    while (*message != 0){
        ssd1306_drawChar(i,j,*message); // plot the character at that array location
        i = i+5; // increment horizontal position
        if (i>=hori_max){
            i=0; // reset horizontal position
            j = j+8; // increment vertical position
        }
        if (j>=32){
            return;
        }
